...
var checkTTCEncodingStringTests = []struct {
	version byte
	in      string
	out     string
}{
...
func TestTTCAddress(t *testing.T) {
	fmt.Println(" TTC address Encoding/Decoding test......")

	for x, test := range checkTTCEncodingStringTests {
		// test encoding
		var data string
		...
		if res := TTCEncode(in_byte_array, test.version); res != test.out {
			fmt.Printf("Base58 str len: %d\n", len(res))
			t.Errorf("TTCEncodeAddress test #%d failed: got %s, want: %s", x, res, test.out)
		}

		// test decoding
		res1, version, err := TTCDecode(test.out)

		res := hex.EncodeToString(res1)

		if err != nil {
			t.Errorf("TTCDecode test #%d failed with err: %v", x, err)
		} else if version != addressPrefix && version != contractPrefix {
			t.Errorf("TTCDecode test #%d failed: got version: %d want: %d", x, version, test.version)
		} else if string(res) != data {
			t.Errorf("TTCDecode test #%d failed: got: %s want: %s", x, res, test.in)
		}
	}

	// test the two decoding failure cases
	// case 1: checksum error
	_, _, err := TTCDecode("3MNQE1Y")
	if err != ErrChecksum {
		t.Error("TTCDecode test failed, expected ErrChecksum")
	}
